          ...
          return (
            <Link
              ...
              className={`flex items-center px-4 py-3.5 rounded-xl transition-all font-bold ${
                isActive
                  ? isDark 
                    ? 'bg-gradient-to-r from-[#e11d48] to-[#9f1239] text-white shadow-[0_4px_20px_rgba(225,29,72,0.4)]'
      ...
      <div className={`p-6 border-t ${isDark ? 'border-gray-800' : 'border-gray-50'}`}>
        <button
          ...
          className={`flex w-full items-center px-4 py-3 text-sm font-bold transition-colors ${
            isDark ? 'text-gray-500 hover:text-red-500' : 'text-gray-400 hover:text-red-600'
          }`}
        >
          <svg className="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
